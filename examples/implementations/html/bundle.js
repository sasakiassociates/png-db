...
dbCrossCheck.loadAllRecordsFromJson(dir + "/full.json").then(function () {
    crossCheckLoaded = true;
    _showTable(dbCrossCheck, $crossCheckPanel);
    _crossCheckData();
});

var _crossCheckData = function _crossCheckData() {
    ...
    var report_err = function report_err(i, fieldName, err) {
    ...
    db.records.forEach(function (record, i) {
        ...
        Object.keys(db.fields).forEach(function (fieldName) {
            if (db.fields[fieldName].dataLoaded) {
                var jsonVal = matchingRecord[fieldName];
...
var _showTable = function _showTable(pngDb, $panel) {
    $panel.empty();
...
db.load(dir + "/test-db.json").then(function () {
    Object.keys(db.fields).forEach(function (fieldName) {
        $('<button>').text(fieldName).appendTo($panel).click(function () {
            db.loadField(fieldName).then(function () {
